...
export default function CycleCountPage() {
  const { parts: allParts, dataSource, updatedAt } = useMergedParts();
    ...
    <div style={{ minHeight: "100dvh", background: DS.bg, fontFamily: FONT, color: DS.onSurface, display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: DS.onSurface }}>
                {dataSource === "indexeddb"
                  ? `已載入主檔資料（${allParts.length.toLocaleString()} 筆）`
                  : "Demo 展示資料"}
              </span>
            </div>
            <button
          ...
          {showVerify && (
            <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${DS.border}` }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                {catSummary.map(([catKey, count]) => {
                  const info = ERP_CATEGORY[catKey] ?? { label: catKey, color: DS.outline, bg: DS.bg };
                  return (
                    <div key={catKey} style={{
                      ...
                      <span style={{ fontSize: 13, fontWeight: 700, fontFamily: MONO, color: info.color }}>
              ...
              <div style={{
                ...
                <span style={{ fontSize: 14, fontWeight: 800, fontFamily: MONO }}>
                  {allParts.length.toLocaleString()} 筆
                </span>
              </div>
            </div>
          )}
      ...
      <div style={{
        ...
        <Link href="/erp/mobile/count" style={{ textDecoration: "none", display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 600, color: DS.secondaryContainer }}>平日抽盤</span>
        </Link>
      </div>
    </div>
  );
