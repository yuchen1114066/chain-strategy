...
export default function ReceivingChecklistClient({ po }: { po: IncomingPO }) {
  ...
  return (
    <div className="p-6 space-y-5 max-w-5xl">
      <Link href="/wms/receiving" className="text-cyan-700 hover:underline text-sm">← 回收貨清單</Link>
                ...
                return (
                  <div key={sc.key} className={`rounded-lg border bg-white p-3 ${
                    ...
                    {currentStepDef.key === "iqc" && sc.key === "dimension" && (
                      <Link href="/wms/spc-shaft" className="inline-block mt-2 text-xs text-cyan-700 hover:underline">
      ...
      {rec.verdict === "FAIL_HOLD" && (
        <section className="rounded-xl border-2 border-rose-500 bg-rose-50 p-5">
          ...
          <div className="mt-3 text-[11px] text-slate-600">
            ...
            <Link href="/wms" title="standalone — 跨模組連結已停用" className="text-cyan-700 underline mx-1">決策閉環中心</Link>
            處理（退貨 / 折讓 / 部分接受 / 換批）。
          </div>
        </section>
      )}
